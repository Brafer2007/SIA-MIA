...
public class MensajeController {
    ...
    @GetMapping("/chats-info/{ficha}")
    public ResponseEntity<List<Map<String, Object>>> obtenerChatsInfoPorFicha(@PathVariable String ficha) {
        List<Programacion> lista = programacionRepository.findByNombreFicha(ficha);
        java.util.Map<String, Map<String, Object>> salasMap = new java.util.LinkedHashMap<>();
        for (Programacion p : lista) {
            String nombre = p.getNombreFicha();
            if (nombre == null) continue;
            nombre = nombre.replace("–", "-");
            String[] partes = nombre.split("-");
            for (String parte : partes) {
                if (parte.trim().equalsIgnoreCase(ficha.trim()) && p.getInstructor() != null) {
                    String key = parte.trim() + "|" + p.getInstructor().getId();
                    if (!salasMap.containsKey(key)) {
                        Map<String, Object> info = new java.util.LinkedHashMap<>();
                        info.put("sala", key);
                        info.put("ficha", parte.trim());
                        info.put("instructorId", p.getInstructor().getId());
                        String nombreInstructor = p.getInstructor().getUsuario() != null
                                ? p.getInstructor().getUsuario().getNombres() + " " + p.getInstructor().getUsuario().getApellidos()
                                : "Instructor " + p.getInstructor().getId();
                        info.put("nombreInstructor", nombreInstructor);
                        info.put("curso", p.getCurso() != null ? p.getCurso() : "");
                        salasMap.put(key, info);
                    }
                }
            }
        }
        return ResponseEntity.ok(new java.util.ArrayList<>(salasMap.values()));
    }
