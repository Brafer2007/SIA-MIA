...
public class DashboardController {
    ...
    public String dashboard(HttpSession session) {
        ...
        return switch (perfil) {
            ...
            case 5 -> "redirect:/seguridad/dashboard";
            default -> "redirect:/dashboard/invitado";
        };
    }
    ...
    public String dashboardAprendiz(HttpSession session, Model model) {
        ...
        if (aprendiz.getFichaFormacion() == null)
            aprendiz.setFichaFormacion("");
        if (aprendiz.getProgramaFormacion() == null)
            aprendiz.setProgramaFormacion("");

        // Usar el campo perfilCompleto de la BD, o calcular basado en los datos si no
        // existe
        boolean aprendizCompleto = aprendiz.getPerfilCompleto() != null && aprendiz.getPerfilCompleto() == 1;
        if (!aprendizCompleto && !aprendiz.getFichaFormacion().isEmpty()
        ...
        try {
            equipos = ((com.example.SIA.service.EquipoService) org.springframework.beans.factory.BeanFactoryUtils
        ...
        // ✅ Horario
        if (aprendizCompleto) {
            List<com.example.SIA.entity.Programacion> horario = programacionRepository
                    .findByNombreFicha(aprendiz.getFichaFormacion());
    ...
    // PANEL INVITADO
    // ============================================================
    @GetMapping("/dashboard/invitado")
    public String dashboardInvitado(HttpSession session, Model model) {
        Integer idPerfil = (Integer) session.getAttribute("id_perfil");
        if (idPerfil == null) return "redirect:/login";
