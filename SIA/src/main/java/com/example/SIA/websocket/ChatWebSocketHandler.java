...
public class ChatWebSocketHandler extends TextWebSocketHandler {
    ...
    // Map key: sala = fichaReal + "|" + instructorId (for backward compat with
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        ...
        if (parts.length >= 4) {
            ...
            logger.info("WS connect (instructor) session={} fichaRaw={} decodedFichas={} instructorId={} salas={}",
                    session.getId(), fichaRaw, fichasReales, instructorId, salas);
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        logger.info("Incoming WS message session={} payloadFicha={} payloadSala={} texto='{}'", session.getId(),
                mensaje.getFicha(), mensaje.getSala(), mensaje.getMensaje());
        ...
        if (salas != null && !salas.isEmpty()) {
            // sender is instructor (connected to one or more salas) — use session salas,
            // ignore payload sala
            ...
            // sender may be apprentice without selected instructor; broadcast to all salas
            // that match the ficha
            ...
            if (ficha != null) {
                for (String salaKey : sesionesPorSala.keySet()) {
                    if (salaKey.startsWith(ficha + "|"))
                        targetSalas.add(salaKey);
                }
            }
        ...
        if (salas != null && !salas.isEmpty()) {
            ...
            if (!fichaOriginal.isEmpty() && !instructorId.isEmpty()) {
                ...
                for (String fichaParte : fichasReales) {
                    ...
                    logger.info("Persisted instructor mensaje id={} sala={} ficha={} fecha={}", p.getIdMensaje(),
                            salaParte, fichaParte, p.getFechaEnvio());
                }
            }
            ...
            logger.info("Persisted apprentice GROUP CHAT mensaje id={} sala={} ficha={} fecha={}", p.getIdMensaje(),
                    instructorGroupSala, ficha, p.getFechaEnvio());
        } else if (!targetSalas.isEmpty()) {
            // Apprentice: persist using the targetSalas (which may already be expanded
            // ranges)
            ...
            for (String sala : targetSalas) {
                ...
                logger.info("Persisted apprentice mensaje id={} sala={} ficha={} fecha={}", p.getIdMensaje(), sala,
                        fichaPart, p.getFechaEnvio());
            }
        } else {
            logger.info(
                    "DEBUG: Apprentice message but NO targetSalas and NO instructorGroupSala, creating ficha-only entry");
            ...
            if (ficha != null) {
                ...
                logger.info("Persisted apprentice (no instructor) mensaje id={} ficha={} fecha={}", p.getIdMensaje(),
                        ficha, p.getFechaEnvio());
            }
        }

        // Build a single map session -> message (one message per session) to avoid
        // duplicates
        ...
        if (instructorGroupSala != null) {
            ...
            if (msgEntry != null) {
                ...
                List<WebSocketSession> sesionesGroup = sesionesPorInstructor.getOrDefault(instructorGroupSala,
                        List.of());
                logger.info("DEBUG: Group chat sala={} has {} apprentice sessions", instructorGroupSala,
                        sesionesGroup.size());
                ...
                // ALSO send to ALL instructors teaching this instructorId (any
                // ficha|instructorId pattern)
                for (Map.Entry<String, List<WebSocketSession>> e : sesionesPorSala.entrySet()) {
                    ...
                    if (salaKey.endsWith("|" + instructorGroupSala)) {
                        ...
                        logger.info("DEBUG: Group chat sending to instructor sala={} with {} sessions", salaKey,
                                instructorSessions.size());
        ...
        // Then, assign apprentice subscribers per ficha (for old format or ficha-only
        // mode)
        for (Map.Entry<String, MensajeGrupo> entry : persistedPorSala.entrySet()) {
            ...
            if (!key.contains("|") && !key.equals(instructorGroupSala)) {
                ...
                for (String fichaIndividual : fichasExpanded) {
                    ...
                    logger.info("DEBUG: Apprentice old-format sala={} ficha={} (expanded from {}) has {} sessions", key,
                            fichaIndividual, fichaDeSala, sesionesApr.size());
                    for (WebSocketSession s : sesionesApr) {
                        if (!s.isOpen())
                            continue;
                        sessionToMessage.putIfAbsent(s, msgEntry);
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        ...
        if (salas != null && !salas.isEmpty()) {
            ...
            if (instructorId != null) {
                ...
                if (sesiones != null)
                    sesiones.remove(session);
                logger.info("WS close (apprentice group chat) session={} instructorId={}", session.getId(),
                        instructorId);
            }
        }
