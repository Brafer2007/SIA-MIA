...
public class SeguridadController {
    ...
    public String dashboardSeguridad(HttpSession session, Model model) {
        // Validación de sesión (ajusta el perfil de seguridad según tu caso, asumo
        // perfil 4 para "Seguridad")
        Integer perfil = (Integer) session.getAttribute("id_perfil");
        if (perfil == null) {
            return "redirect:/login";
        }
        // Validamos si tiene perfil de seguridad (4 = Seguridad, o el nro que utilices)
        // Por ahora lo permitimos y mandamos a la vista
    ...
    public ResponseEntity<?> buscarUsuario(@RequestParam String documento) {
        ...
        // Determinar si el usuario ya está dentro (último registro es INGRESO sin SALIDA posterior)
    ...
    public ResponseEntity<?> registrarAcceso(@RequestBody Map<String, Object> payload) {
        try {
            ...
            String tipo = payload.get("tipo").toString(); // "INGRESO" o "SALIDA"
            String metodo = payload.get("metodo").toString(); // "QR" o "MANUAL"
            ...
            return ResponseEntity
                    .ok(Map.of("mensaje", "Registro de " + tipo + " exitoso", "fecha", registro.getFechaHora()));
        } catch (Exception e) {
    ...
    // ==========================================
    // API: GESTIÓN DE AMBIENTES (Para que Seguridad vea el listado)
    // ==========================================
    ...
    public ResponseEntity<?> enviarSolicitud(@RequestBody Map<String, Object> payload) {
        try {
            ...
            Instructor instructor = instructorRepository.findByUsuario_IdUsuario(idUsuario)
                    .orElse(null);
            if (instructor == null) {
                return ResponseEntity.badRequest()
            ...
            String nombreInstructor = instructor.getUsuario() != null
                    ? instructor.getUsuario().getNombres()
                    : "Instructor";

            NotificacionDTO msg = new NotificacionDTO(
                    "solicitud_ambiente",
                    "Solicitud de Aula",
                    "El instructor " + nombreInstructor + " solicita " + tipo + " del aula " + aula,
                    "Sistema",
                    "N/A");
            ...
            return ResponseEntity.internalServerError().body(Map.of("error", "Error interno al enviar solicitud: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> actualizarEstadoSolicitud(@PathVariable Long id, @RequestParam String estado) {
        ...
        sol.setEstado(estado); // "APROBADA" o "RECHAZADA"
        solicitudAmbienteRepository.save(sol);

        // Notificar al instructor del cambio de estado
        NotificacionDTO msg = new NotificacionDTO(
                "estado_solicitud",
                "Actualización de Solicitud de Aula",
                "Tu solicitud para el aula " + sol.getAula() + " ha sido " + estado,
                "Seguridad",
                "N/A");
        msg.setIdMensaje(sol.getId());
        msg.setSala(estado); // reutilizamos sala para pasar el nuevo estado
    ...
    public ResponseEntity<?> reporteDiario(@RequestParam String fecha) {
        try {
            ...
            return ResponseEntity.badRequest().body(Map.of("error", "Fecha inválida: " + e.getMessage()));
        }
    }
}
